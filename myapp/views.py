...
def search_list(request):
    if 'cname' in request.GET:
        ...
        print(cname)
        # resultList = students.objects.filter(cname=cname)   #絕對查詢
        resultList = students.objects.filter(cname__contains=cname)   #模糊查詢
    ...
    # return HttpResponse("Hello, this is the search lisr page.")
    errormessage=""
    # resultList=[]   #模擬資料為空
...
def index(request):
    if 'site_search' in request.GET:
        ...
        site_search = site_search.strip()       ##.strip()去除前後空白
        keywords = site_search.split()          ##.split()以空白分割成多個關鍵字
        ...
        #多個關鍵字+搜尋多個欄位 (github copilot)
        # query = Q()
        # for keyword in keywords:
        #     query |= Q(cname__contains=keyword) | Q(cbirthday__contains=keyword) | Q(cemail__contains=keyword) | Q(cphone__contains=keyword) | Q(caddr__contains=keyword)
        # resultList = students.objects.filter(query).order_by('cid')
        # resultList = []

        #多個關鍵字+搜尋多個欄位 (chatgpt)
        q_objects = Q()
    ...
    page_obj = paginator.get_page(page_number)  #獲取當前頁碼的資料
